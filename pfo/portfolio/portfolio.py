...
class Portfolio(object):
    ...
    def __init__(self, data: pd.DataFrame, weights=None, risk_free_rate=0.0425, freq=252):
        ...
        if len(self._weights) < len(self._data.columns) or len(self._weights) > len(self._data.columns):
    ...
    def plot_mc_simulation(self):
        self._mc_portfolios.plot.scatter(x='Volatility', y='Returns', marker='o', s=10, alpha=0.3, grid=True,
                                         figsize=[16, 10])
        plt.scatter(self._mc_min_vol_port[1], self._mc_min_vol_port[0], color='r', marker='*', s=500)
        plt.scatter(self._mc_min_downside_vol_port[1], self._mc_min_downside_vol_port[0], color='c', marker='*', s=500)
        plt.scatter(self._mc_max_sharpe_port[1], self._mc_max_sharpe_port[0], color='g', marker='*', s=500)
        plt.scatter(self._mc_max_sortino_port[1], self._mc_max_sortino_port[0], color='y', marker='*', s=500)
    ...
    def mc_simulation(self, num_portfolios=10000):
        self._mc_portfolios = mc_random_portfolios(data=self._data, risk_free_rate=self._risk_free_rate,
                                                   num_portfolios=num_portfolios, freq=self._freq)

        self._mc_min_vol_port = self._mc_portfolios.iloc[self._mc_portfolios['Volatility'].idxmin()]
        self._mc_min_downside_vol_port = self._mc_portfolios.iloc[self._mc_portfolios['Down. Volatility'].idxmin()]
        self._mc_max_sharpe_port = self._mc_portfolios.iloc[self._mc_portfolios['Sharp Ratio'].idxmax()]
        self._mc_max_sortino_port = self._mc_portfolios.iloc[self._mc_portfolios['Sortino Ratio'].idxmax()]
        ...
        self._mc_simulations_results = pd.concat(
            [self._mc_min_vol_port, self._mc_min_downside_vol_port, self._mc_max_sharpe_port,
